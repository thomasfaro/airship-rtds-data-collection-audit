...
import { purgeExpiredCaptures } from "./storage/retention.js";
...
// Saved captures hold a client's data, so they expire. Startup is the only trigger: this is a
// tool someone starts by hand, and a scheduler would be machinery for nothing. Deletions are
// announced — silence is right for a no-op, wrong for removing someone's work.
try {
  const purged = purgeExpiredCaptures();
  if (purged.files > 0) {
    console.log(
      `[retention] Removed ${purged.audits} saved analysis(es) and ${purged.liveFiles} live file(s) ` +
        `older than ${purged.retentionDays} days.`,
    );
  }
} catch (error) {
  console.warn(`[retention] Skipped expiry of stored captures: ${error.message}`);
}
