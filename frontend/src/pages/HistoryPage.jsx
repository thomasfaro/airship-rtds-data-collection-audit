...
import { deleteHistoryItem, fetchHistory, fetchHistoryReport } from "../services/historyApi.js";
...
function HistoryRow({ item, onOpen, onDelete, opening }) {
  return (
    ...
    <div className="space-y-4">
      <header>
        ...
        <p className="mt-1 text-sm text-airship-muted">
          Every capture is saved on this machine. Reopen one to review its coverage or download the
          tagging plan again.
        </p>
      </header>
      ...
      <section className="card overflow-hidden">
        {loading ? (
          ...
          <p className="px-4 py-4 text-sm text-airship-muted">
            No saved capture yet. Run one from the Capture screen.
          </p>
        ) : (
      ...
      <ConfirmDialog
        ...
        message={`Delete the analysis for "${pendingDelete?.profile ?? pendingDelete?.name}" saved ${formatDateTime(pendingDelete?.savedAt)}? This cannot be undone.`}
