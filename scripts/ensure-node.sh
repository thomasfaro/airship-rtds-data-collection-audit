...
#
# The private copy stays inside the app folder: no administrator password, nothing
...
RTDS_DCA_MIN_NODE_MAJOR="${RTDS_DCA_MIN_NODE_MAJOR:-20}"
# Bump this to move to a newer Node line; any nodejs.org/dist channel name works.
RTDS_DCA_NODE_CHANNEL="${RTDS_DCA_NODE_CHANNEL:-latest-v22.x}"

_dca_node_is_recent_enough() {
  ...
  [[ "$major" -ge "$RTDS_DCA_MIN_NODE_MAJOR" ]]
}

# Version managers keep their Node outside the default PATH.
_dca_load_node_from_managers() {
  local had_u=0
  local had_e=0
...
_dca_install_private_node() {
  local target_dir="$1"
  local platform arch dist_url shasums entry expected filename tmp actual

  if ! platform="$(_dca_node_platform)"; then
    echo "Unsupported operating system: $(uname -s)." >&2
    return 1
  fi
  if ! arch="$(_dca_node_arch)"; then
  ...
  dist_url="https://nodejs.org/dist/${RTDS_DCA_NODE_CHANNEL}"
  ...
  if [[ -z "$entry" ]]; then
    echo "nodejs.org has no ${platform}-${arch} build in the ${RTDS_DCA_NODE_CHANNEL} channel." >&2
    return 1
  fi
  expected="${entry%% *}"
  filename="${entry##* }"
  ...
  if ! actual="$(_dca_sha256 "${tmp}/${filename}")"; then
    rm -rf "$tmp"
...
# Returns 0 once `node` and `npm` on PATH are recent enough, 1 if the user has to act.
ensure_node_available() {
  local root="$1"
  local private_dir="${root}/.node"
  local reply=""

  if _dca_node_is_recent_enough; then
    return 0
  fi
  ...
  if command -v node >/dev/null 2>&1; then
    echo "Node.js $(node -v) is installed, but this app needs version ${RTDS_DCA_MIN_NODE_MAJOR} or later."
  else
  ...
  if [[ "${RTDS_DCA_AUTO_INSTALL_NODE:-0}" == "1" ]]; then
    reply="y"
  elif [[ -t 0 ]]; then
    read -r -p "Install it now? [Y/n] " reply || reply="n"
  else
    echo "Start the launcher from a terminal window to answer this question," >&2
    echo "or install Node.js ${RTDS_DCA_MIN_NODE_MAJOR}+ yourself from https://nodejs.org." >&2
    return 1
  fi
  ...
  case "${reply:-y}" in
    ...
    *)
      echo ""
      echo "Nothing installed. Get Node.js ${RTDS_DCA_MIN_NODE_MAJOR}+ from https://nodejs.org,"
      echo "then start this launcher again."
      return 1
      ;;
  esac

  if ! _dca_install_private_node "$private_dir"; then
    return 1
  fi
