...
- **Saved audits expire after 30 days.** They hold a client's data, so they are removed
  automatically rather than kept forever. Download the tagging plan when you finish and nothing is
  lost — that file is the deliverable. The window is configurable, see the
  [install guide](INSTALL.md#files-to-keep-to-yourself).
- **The exports are client deliverables.** A tagging plan describes a client's taxonomy in detail,
  and its value sheets list values actually seen in the stream — so an email address or a customer
  reference sitting in an attribute lands in the file. Treat it with the same care as anything else
  you would send them, and glance at the value sheets before forwarding it more widely.
