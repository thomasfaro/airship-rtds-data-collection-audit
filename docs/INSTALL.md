...
Two reasons this is the recommended route and not just one option among others. It is the only one that
keeps the app **updating itself**, because the update needs the credentials GitHub Desktop stores when
you clone. And nothing arrives flagged: macOS quarantines everything a browser downloads, and a
...
Two costs. On macOS this means one security prompt the first time you start the app — step 3 explains
how to clear it in two clicks. And a ZIP folder has no link back to the repository, so it **will not
update itself**: every new version is another download, plus copying your `config/` folder across to
keep your projects.
...
**There is nothing to do**, as long as you cloned with GitHub Desktop in step 2. The tool then updates
itself, in two ways that cover each other:
...
- **You installed from a ZIP.** There is no link back to the repository, so nothing above applies:
  download the new ZIP into a new folder and copy your `config/` folder across to keep your projects.
  Copy the whole folder, because it holds both your tokens and the key that reads them — which is
  also why that copy is a copy of your credentials, so keep it off shared drives and mail. Cloning
  with GitHub Desktop avoids the whole exercise.
- **GitHub does not recognise your machine.** The repository is private, so the update needs the
  credentials GitHub Desktop stores when you clone with it. Without them the tool stays on the version
  it has and says nothing — deliberately, because a launcher that stopped to ask for a password would
  be worse. So if the banner never appears, check you are still signed in to GitHub Desktop with an
  account that has access to this repository.
...
| An update banner never appears, on any version | If you cloned: open GitHub Desktop and check you are still signed in with an account that has access to this repository. If you downloaded the ZIP, this is expected — see [Updating](#updating-to-a-newer-version) |
...
- `config/.profiles-key` — the key that reads those tokens, so it matters as much as they do
...
**Saved audits do not pile up forever.** A saved analysis, and any live file you chose to keep, is
removed automatically once it is **30 days old** — they hold a client's data, so keeping them
indefinitely on your machine is not the safe default. The tool says what it removed when it starts.
If you need a longer window, or none at all, put `RTDS_DCA_RETENTION_DAYS=90` — or `0` to keep
everything — in `server/.env`. Export the tagging plan when you finish a capture and nothing is lost:
that file is the deliverable, and it is yours to keep wherever you file client work.

The exported `.xlsx` / `.json` tagging plans need the same care as any other client deliverable, and
for one reason worth knowing: besides the taxonomy, they list **values actually seen in the stream**,
because seeing real values is how a client recognises its own tracking. So whatever a client wrote
into an attribute or an event property can appear there — an email address, a customer reference, a
name. Read the value sheets before forwarding a plan to anyone who would not be given the raw data.
