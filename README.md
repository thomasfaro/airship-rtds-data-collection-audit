...
Two double-clicks, on **macOS** or **Windows**. Get the folder with [GitHub
Desktop](https://desktop.github.com) — the repository page's green **Code** button → **Open with
GitHub Desktop** — then double-click the launcher inside. It installs everything it needs, Node.js
included, opens your browser on the tool, and keeps itself up to date from then on.
...
Nothing to do, provided you cloned it with GitHub Desktop: it picks up the latest version each time it
starts, and while you are using it a banner offers the update with a single button. The version it is
running sits in the top-right corner and is recorded inside every plan you export, so a file found six
months later names the version that produced it.

## Everything stays on your machine

- Tokens are **encrypted at rest**, with a key generated here on first run and kept in its own file
  next to them, readable by your account alone. That protects the profiles file wherever it travels
  without the key — a backup, a sync folder, a copy sent by mistake. It is not a defence against
  someone who already has your session, and moving `config/` to another machine takes the key along
  with the tokens.
...
- What is saved **expires after 30 days**, because it holds a client's data. Export the tagging plan
  and you keep the deliverable; the working copy cleans itself up.
- **Nothing you capture leaves the machine.** The app calls the Airship RTDS endpoint with your
  token, and GitHub for two read-only checks: the SDK release dates the obsolescence report compares
  against, and whether a newer version of this tool exists. Fonts are bundled rather than fetched, so
